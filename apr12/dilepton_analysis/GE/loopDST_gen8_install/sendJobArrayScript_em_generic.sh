...
day=${1}
weight=${2}
submmissionbase=/hera/hades/user/${user}/sub/GE/gen8/dilepton_em_generic_emburqmd_evtclasmultrphi_mineff002_one_minmom_richQa1_buffsize160
submissiondir=${submmissionbase}/loopDST_gen8_install
 nFilesPerJob=200                                                   # number of files to be analyzed by 1 job (default==1)
# nFilesPerJob=1000                                                   # number of files to be analyzed by 1 job (default==1)
    jobscript=${submissiondir}/jobArrayScript.sh                        # exec script (full path, call without dot, set it executable!)
    outputdir=/hera/hades/user/${user}/apr12/analysis_exp/gen8/dilepton_em_w${weight}_mineff002_richQacutg_emburqmd_fm2_min_maxmom_buffsize40_one_generic_multtarget/${day}
pathoutputlog=${outputdir}/out                                  # protocol from batch farm for each file
     filename=apr12_diele_${day}                             # filename of log file if nFilesPerJob > 1 (partnumber will be appended)
#par1=/hera/hades/user/harabasz/myhydralibs-sharedptr-4.9f/install/defall.sh   # optional par1 : environment script
par1=/hera/hades/user/harabasz/myhydralibs-newmixer-4.9f/defall.sh   # optional par1 : environment script
par2=${submissiondir}/anaDilepton_em_generic                               # optional par2 : executable
...
par6=${weight}                                                       # optional par6
par7=                                                       # optional par7
resources="-l h_rt=20:0:0,h_vmem=8G"                            # runtime < 10h, mem < 2GB
jobarrayFile="jobarray_generic_${1}_w${weight}.dat"
